...
// Delay control state
volatile u8 delay_enabled = 0;  // Start with delay OFF
volatile u32 delay_samples = DELAY_SAMPLES_DEFAULT;  // Default delay time
volatile u32 samples_written = 0;  // Track how many samples have been written to buffer
...
// Always-on filter state variables
static int32_t hp_filter_state = 0;  // High-pass filter state
static int32_t agc_gain = 256;       // AGC gain (256 = 100%, starts at full gain)
...
void sampling_ISR() {
    ...
    // ========================================
    // ALWAYS-ON SAFETY EFFECTS
    // ========================================
    
    // 1. HIGH-PASS FILTER (removes low-frequency rumble)
    // Simple one-pole high-pass filter: y[n] = alpha * y[n-1] + alpha * (x[n] - x[n-1])
    // Using fixed-point: alpha = HP_FILTER_COEFF/256
    // This removes DC and low frequencies that cause feedback through guitar body
    ...
    int32_t scaled_signal_before_agc = scaled_signal;  // Save for debug printing
    
    // 3. AUTOMATIC GAIN CONTROL (AGC) / DUCKING (prevents feedback)
    ...
    // 4. INPUT LIMITER (prevents clipping in processing chain)
    ...
    // Write current sample to circular buffer FIRST
    circular_buffer[write_head] = scaled_signal;
    samples_written++;  // Track total samples written

    // Process delay effect only if enabled
    ...
    if (delay_enabled) {
        ...
        // Delay disabled - output dry signal only
        mixed_signal = scaled_signal;
        
        // Still update write_head to keep buffer fresh
        write_head = (write_head + 1) % BUFFER_SIZE;
        // Don't update read_head when delay is off
    }

    // ========================================
    // OUTPUT LIMITER (final safety check before PWM)
    // ========================================
    // Hard limit to PWM range to protect exciter hardware
    ...
    // re-center for PWM (unsigned output between 0 to RESET_VALUE)
    // we add the mid-point of the PWM ticks (RESET_VALUE/2) to turn the signed AC wave into a positive DC wave
    int32_t pwm_sample = output_signal + (RESET_VALUE / 2);

    // Final hard clip to PWM range (safety check)
	...
	// Debug print statements - use to tune safety effect parameters
	count++;
...
void enc_ISR(void *CallbackRef) {
	...
	/* Handle encoder rotation - adjust delay time when delay is enabled */
	if (delay_enabled) {
		if (s_saw_cw) {
			s_saw_cw = 0;
			// Clockwise: Increase delay time
			delay_samples += DELAY_ADJUST_STEP;
			if (delay_samples > DELAY_SAMPLES_MAX) {
				delay_samples = DELAY_SAMPLES_MAX;
			}
			// Recalculate read_head to maintain delay spacing
			read_head = (write_head - delay_samples + BUFFER_SIZE) % BUFFER_SIZE;
		...
		if (s_saw_ccw) {
			s_saw_ccw = 0;
			// Counter-clockwise: Decrease delay time
		...
		// Delay not enabled - just print rotation for debugging
		if (s_saw_cw) {
			s_saw_cw = 0;
			xil_printf("CW turn (delay OFF - press Button 0 to enable)\r\n");
		}
		if (s_saw_ccw) {
			s_saw_ccw = 0;
			xil_printf("CCW turn (delay OFF - press Button 0 to enable)\r\n");
		}
	}
